...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        // [3] x의 최댓값 구하기
        // A 블록이 x개 있으려면:
        // 1) A 블록마다 A가 최소 1개씩 필요하다 x <= countA
        // 2) A 블록들 사이를 B가 끊어줘야 한다 x <= countB + 1
        // 위 두 조건으로 x의 상한을 구해보기
        x = Math.min(countA, countB + 1);
        ...
        long aUsed = Math.min(countA, x * maxA);
        ...
        long bUsed = Math.min(countB, y * maxB);

        // [7] 최종 정답
        // 전체 문자열 길이 = 실제 사용한 A 개수 + 실제 사용한 B 개수
        long answer = aUsed + bUsed;
